...
class AgentController extends Controller
{
    public function index()
    {
        ...
        return view('frontend.agent-list',compact('agents','categories','recentlyAdded'));
    }
    ...
    public function show(User $agent)
    {
        $recentlyAdded = Property::with(['state.stateTranslation','city.cityTranslation','propertyTranslation'])->where('moderation_status',1)->where('status',1)->latest()->take(5)->get();
        $categories = Category::with('categoryTranslation','properties')->where('status',1)->get();
        $properties = Property::with(['propertyDetails','user','category.categoryTranslation','country.countryTranslation','state.stateTranslation','city.cityTranslation','propertyTranslation','propertyTranslationEnglish','image'])->where('moderation_status',1)->where('user_id',$agent->id)->get();

        return view('frontend.agent',compact('agent','recentlyAdded','categories','properties'));
    }
                                        ...
                                        <div class="row">
                                            <div class="col-md-5 col-sm-12">
                                                <div class="single-team-member agent-item">
                                                    <img src="storage/users/'.$agent->image.'" alt="Image">
                                                </div>
                                            </div>
